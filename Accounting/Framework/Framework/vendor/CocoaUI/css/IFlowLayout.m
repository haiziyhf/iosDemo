...
- (void)layout_once{
	...
	for(IView *sub in _view.subs){
		...
		[sub updateFrame];
		//NSLog(@"%@ position: %@", sub.name, NSStringFromCGRect(sub.style.rect));
	}
...
- (void)place:(IView *)view{
	...
	if(style.ratioWidth > 0){
		...
		//NSLog(@"x2=%f, x=%f, margin-left: %f, margin-right: %f", _x2, _x, style.margin.left, style.margin.right);
		//NSLog(@"left=%lu, right=%lu", _leftPoints.count, _rightPoints.count);
	}
	...
	while(1){
		if(!style.resizeNone){
			...
			if(style.w > 0){
				//NSLog(@"%@ before w=%f, h=%f, _x=%f, x2=%f", view.name, style.w, style.h, _x, _x2);
				view.level += 1;
				//NSLog(@"%s %d call layout %@", __func__, __LINE__, view.name);
				[view layout];
				view.level -= 1;
				//NSLog(@"%@ after w=%f, h=%f, _x=%f, x2=%f", view.name, style.w, style.h, _x, _x2);
				if(style.w == 0){
					return;
				}
			}
	...
	if(style.resizeWidth){
		//NSLog(@"%@ place, x=%f, y=%f, w=%f, h=%f", view.name, style.x, style.y, style.w, style.h);
	}
