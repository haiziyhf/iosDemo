...
- (void)cell:(ICell *)cell didResizeHeightDelta:(CGFloat)delta{
	...
	//NSLog(@"%s %d %.1f height: %.1f", __func__, (int)index, delta, _contentFrame.size.height);
	for(NSUInteger i=index; i<_cells.count; i++){
		ICell *cell = [_cells objectAtIndex:i];
		if(i != index){
			cell.y += delta;
...
- (void)removeVisibleCellAtIndex:(NSUInteger)index{
	...
	if(cell.tag){
		...
		if(views.count < 3 && cell.view){
			...
			//NSLog(@"enqueue cell for tag: %@, count: %d", cell.tag, (int)views.count);
		}
		cell.contentView = nil;
		cell.view = nil;
	}
...
- (void)layoutViews{
	...
	//NSLog(@"%s frame: %.1f, offset: %.1f, size: %.1f, inset: %@", __func__, _scrollView.frame.size.height, _scrollView.contentOffset.y, _contentFrame.size.height, NSStringFromUIEdgeInsets(_scrollView.contentInset));
...
- (void)constructVisibleCells{
	...
	//NSLog(@"visible.index: [%d, %d]=>[%d, %d]", (int)_visibleCellIndexMin, (int)_visibleCellIndexMax, (int)minIndex, (int)maxIndex);
