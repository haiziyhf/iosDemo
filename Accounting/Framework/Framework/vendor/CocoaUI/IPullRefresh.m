...
- (CGFloat)headerVisibleRate{
	if(_headerView && _headerView.frame.size.height > 0){
		...
		//NSLog(@"header: visibleHeight=%f height=%f", visibleHeight, _headerView.frame.size.height);
...
- (CGFloat)footerVisibleRate{
	if(_footerView && _footerView.frame.size.height > 0){
		...
		//NSLog(@"footer: visibleHeight=%f height=%f", visibleHeight, _footerView.frame.size.height);
		//NSLog(@"footer.frame: %@, content: %f, offset: %f", NSStringFromCGRect(_footerView.frame), _scrollView.contentSize.height, _scrollView.contentOffset.y);
...
- (void)scrollViewDidScroll:(UIScrollView*)scrollView{
	//NSLog(@"scroll.offset.y = %f", scrollView.contentOffset.y);
...
- (void)beginRefresh:(IView *)view{
	...
	if(!animation){
		...
		[UIView animateWithDuration:0.2 animations:^(){
			...
			//NSLog(@"inset %@, offset: %@", NSStringFromUIEdgeInsets(_scrollView.contentInset), NSStringFromCGPoint(_scrollView.contentOffset));
